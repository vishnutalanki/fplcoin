...
def getJSONForTransaction(transactionHash):
	...
	transaction = fplcoin.db.doQuery("SELECT hash,timestamp from transactions WHERE hash = ?", (transactionHash,), result='one')
	...
	inputs = fplcoin.db.doQuery("SELECT previousOutput, publicKey, timestamp, signature from transactions_inputs WHERE transactionHash = ?", (transactionHash,), result='all')
	...
	outputs = fplcoin.db.doQuery("SELECT amount, address, outputHash from transactions_outputs WHERE transactionHash = ?", (transactionHash,), result='all')
...
def addTransactionJSON(data):
	...
	for oldInput in transaction['inputs']:
		...
		# input must exist
		amount, address = fplcoin.db.doQuery("SELECT amount, address from transactions_outputs WHERE outputHash = ?", (outputHash,), result='one')
		...
		publicKey = fplcoin.wallets.decompressPublicKey(publicKey)

		signature = fplcoin.ecc.verify_signature(publicKey, message, signature)
			...
			if address == myAddress:
				...
				if myMoney >= transferAmount:
					...
					for oldOutput in usedOutputs:
						...
						signature = fplcoin.ecc.sign_message(int(privateKey), messageToSign)

						correct = fplcoin.ecc.verify_signature(fplcoin.wallets.decompressPublicKey(publicKey), messageToSign, signature)
						# assert correct == True
					...
					fplcoin.network.broadcastTransaction(transactionJSON)
...
def createTransactionConfirmation(transactionHash, timestamp):
	...
	while True:
		...
		solution = fplcoin.hasher(transactionValue + addition).hexdigest()
...
def addConfirmationCSV(data):
	...
	maxDifficulty = fplcoin.db.doQuery("select max(difficulty) from confirmations where transactionHash = ?", (transactionHash,), result='one')[0]
	
	if maxDifficulty >= difficulty:
		return
	
	timestamp = fplcoin.db.doQuery("select timestamp from transactions where hash = ?", (str(transactionHash),), result='one')[0]
	...
	if timestamp:
		...
		if solution.count('0') == difficulty:
			...
			fplcoin.db.doQuery('DELETE FROM confirmations WHERE transactionHash = ?', (transactionHash, ), result='none')
