...
class fplcoinNetwork(object):
	""" Class voor fplcoin networking """
	...
	def startNetworking(self):
		...
		fplcoin.threader.startBackgroundThread(self.runServer)
	...
	def handleServerInput(self, data, ip):
		''' Handle fplcoin packets '''


		# reply to broadcast packets
		if hasPacketPrefix(data, packet_sync_request):
			# todo: do proper sync, not send everything
			transactions = fplcoin.db.doQuery("SELECT hash from transactions WHERE hash != 'd34db33f'", result='all') # everything except genesis transaction
			for transaction in transactions:
				transactionHash = transaction[0]

				# send transaction
				transactionJSON = fplcoin.transactions.getJSONForTransaction(transactionHash)
				fplcoin.network.sendToNode(ip, packet_payment_new + zlib.compress(transactionJSON) )
				
				# send current confirmation
				difficulty,addition = fplcoin.db.doQuery("select difficulty,addition from confirmations where transactionHash = ?", (transactionHash,), result='one')
				data = "%s,%s,%s" % (transactionHash,str(difficulty),addition)
				fplcoin.network.sendToNode(ip, packet_payment_confirmation + data)


		if hasPacketPrefix(data, packet_payment_new):
			fplcoin.transactions.addTransactionJSON( zlib.decompress(data[len(packet_payment_new):]) )

		if hasPacketPrefix(data, packet_payment_confirmation):
			fplcoin.transactions.addConfirmationCSV(data[len(packet_payment_confirmation):])
			

	def runServer(self):
		''' Runs fplcoin UDP server '''
